...
def make_windowed_data(data, start, end, window_size=1):
    ...
    for sentence, labels in data:
        ...
        #     if i == 0:
        #         st = [x for x in start]
        #         wo = [x for x in sentence[i]]
        #         if len(sentence)>1:
        #             wo2 =[x for x in sentence[i+1]]
        #             window = st+wo+wo2
        #         else:
        #             window = st+wo
        #
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup(embed, ids=self.input_placeholder) # input_placeholder 에 나온부분들만 lookup, [batch,n_window_features,embed_size]
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.get_variable('b1',shape=[self.config.hidden_size,],dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        W = tf.get_variable('W1',shape=[self.config.n_window_features*self.config.embed_size,self.config.hidden_size],
                            dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())

        b2 = tf.get_variable('b2', shape=[self.config.n_classes,],dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
