...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        sentence_re = sentence + [zero_vector] * (max_length - len(sentence))
        re_labels = labels + [zero_label]* (max_length - len(sentence))
        mask = [True if i<len(sentence) else False for i in xrange(max_length)]
        ret.append((sentence_re[:max_length],re_labels[:max_length],mask[:max_length]))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_drop = tf.nn.dropout(output,dropout_rate)
    ...
    def add_loss_op(self, preds):
        ...
        b_loss = tf.boolean_mask(loss, self.mask_placeholder)
