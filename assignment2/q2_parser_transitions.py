...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        while len(batch)>0:
            ...
            for i in xrange(min(batch_size, len(batch))):
                batch[i].parse_step(transitions[i])
            batch = [p for p in batch if not (len(p.stack) == 1 and len(p.buffer) == 0)]
        #print batch
        unfinished_parses = unfinished_parses[batch_size:]
        #print 'unfi',len(unfinished_parses)
    dependencies = [p.dependencies for p in partial_parses]


    # partial_parses = [PartialParse(s) for s in sentences]
    # unfinished_parse = partial_parses #operation copy
    #
    ...
    #         print len(minibatch),len(unfinished_parse)
    #
